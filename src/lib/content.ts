...
async function load(): Promise<Content> {
  ...
  const doc = await client.fetch('*[_type == "siteSettings"] | order(_updatedAt desc)[0]{..., "heroImage":heroImage.asset->url,"portraitImage":portraitImage.asset->url,"clinicImage":clinicImage.asset->url,"treatmentImage":treatmentImage.asset->url}');
